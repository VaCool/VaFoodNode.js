...
$(document).ready(function() { // зaпускaем скрипт пoсле зaгрузки всех элементoв
    ...
    function getDessert(){
        $.ajax({
            ...
            pizzaGallery.forEach(function(dessert) {
                var $pizzaTemplate = $("#template > div").clone();
                $pizzaTemplate.find("[data-class]").attr("class","category " + dessert.categories);
                $pizzaTemplate.find("[data-id]").attr("href","#" + dessert.dessert_id);
                $pizzaTemplate.find("[data-url-small]").attr("src", dessert.URL);
                $pizzaTemplate.find("[data-name-first]").text(dessert.name);
                $pizzaTemplate.find("[data-weight]").text(dessert.weight);
                $pizzaTemplate.find("[data-price]").text(dessert.price);
                $pizzaTemplate.find("[data-link]").attr("id",dessert.dessert_id);
                $pizzaTemplate.find("[data-url-big]").attr("src", dessert.URL);          
                $pizzaTemplate.find("[data-name-second]").text(dessert.name);
                $pizzaTemplate.find("[data-consist]").text(dessert.consist);              
...
del.click(function(event){ 
  ...
  $.ajax({
        ...
        data:  
        {
            dessert_id: $(this).parent().parent().find('.modal_div').attr('id'),
