...
$(document).ready(function() { // зaпускaем скрипт пoсле зaгрузки всех элементoв
    ...
    function getRisotto(){
        $.ajax({
            ...
            pizzaGallery.forEach(function(risotto) {
                var $pizzaTemplate = $("#template > div").clone();
                $pizzaTemplate.find("[data-class]").attr("class","category " + risotto.categories);
                $pizzaTemplate.find("[data-id]").attr("href","#" + risotto.risotto_id);
                $pizzaTemplate.find("[data-url-small]").attr("src", risotto.URL);
                $pizzaTemplate.find("[data-name-first]").text(risotto.name);
                $pizzaTemplate.find("[data-weight]").text(risotto.weight);
                $pizzaTemplate.find("[data-price]").text(risotto.price);
                $pizzaTemplate.find("[data-link]").attr("id",risotto.risotto_id);
                $pizzaTemplate.find("[data-url-big]").attr("src", risotto.URL);          
                $pizzaTemplate.find("[data-name-second]").text(risotto.name);
                $pizzaTemplate.find("[data-consist]").text(risotto.consist);              
...
del.click(function(event){ 
  ...
  $.ajax({
        ...
        data:  
        {
            risotto_id: $(this).parent().parent().find('.modal_div').attr('id'),
